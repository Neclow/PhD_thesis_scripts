...
# ----------------------------------------------------------------------------
# 1. Create two gene trees: B is the discordant taxon
# ----------------------------------------------------------------------------

# Tree for Region 1: ((A,C),((B,D),E))
# B clusters deeply with D
tree1 <- read.tree(
  text = "((A:0.15,C:0.12):0.2,((B:0.08,D:0.1):0.12,E:0.18):0.15);"
)

# Tree for Region 2: ((A,(B,C)),(D,E))
# B has recombined - now clusters with C instead of D
# B has much longer branch in Region 2 (accelerated evolution)
tree2 <- read.tree(
...
get_rtt <- function(tree) {
  ...
  distances <- dist.nodes(tree)[1:length(tree$tip.label), root_node]
...
plot_tanglegram <- function() {
  ...
  edgelabels.cophylo(
    ...
    which = "left", cex = 1.2, frame = "none", adj = c(0.5, -0.3)
  )
  edgelabels.cophylo(
    round(cophy$trees[[2]]$edge.length, 2),
    which = "right", cex = 1.2, frame = "none", adj = c(0.5, -0.3)
  )
...
cat("TDI_B = MCI(Tree 1, Tree 2) - MCI(Tree 1 \\ B, Tree 2 \\ B) =",
    round(mci, 3), "-", round(mci_reduced, 3), "=", round(tdi_B, 3), "\n")
